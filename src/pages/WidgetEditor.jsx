// WidgetEditor.jsx – Editor zur Konfiguration des Feedback-Widgets
...
export default function WidgetEditor() {
  ...
  const iframeCode = useMemo(() => {
    const params = new URLSearchParams({
      ...
      widgetStylePreset,
      stylePreset,
      headingBold: headingStyles.bold,
      headingItalic: headingStyles.italic,
      headingUnderline: headingStyles.underline,
      headingColor: headingStyles.color,
      logoUrl
    });

    return `<iframe 
  src="https://www.ki-partner24.de/embed/${firmaId}?${params.toString()}"
  width="100%"
  height="360"
...
/>`;
    ...
    stylePreset,
    headingStyles.bold,
    headingStyles.italic,
    headingStyles.underline,
    headingStyles.color,
    logoUrl
  ]);
      ...
      <div className="w-full max-w-2xl">
        ...
        {activeTab === "colors" && (
          <div className="grid gap-4">
            ...
            <label>
              Pfeilfarbe:
              <input type="color" className="w-full p-2 border" value={arrowColor} onChange={(e) => setArrowColor(e.target.value)} />
            </label>
            <label>
              Pfeil-Hintergrundfarbe:
              <input type="color" className="w-full p-2 border" value={arrowBgColor} onChange={(e) => setArrowBgColor(e.target.value)} />
            </label>
          </div>
        )}
          ...
          <div className="grid gap-4">
            ...
            <fieldset className="border p-4 rounded space-y-2">
              ...
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={headingStyles.bold} onChange={(e) => setHeadingStyles({ ...headingStyles, bold: e.target.checked })} /> Fett
              </label>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={headingStyles.italic} onChange={(e) => setHeadingStyles({ ...headingStyles, italic: e.target.checked })} /> Kursiv
              </label>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={headingStyles.underline} onChange={(e) => setHeadingStyles({ ...headingStyles, underline: e.target.checked })} /> Unterstrichen
              </label>
            </fieldset>
          </div>
        )}
        ...
        {activeTab === "branding" && (
          <div className="grid gap-4">
            <label>
              Widget-Titel:
              <input type="text" className="w-full p-2 border" value={customTitle} onChange={(e) => setCustomTitle(e.target.value)} placeholder="Unsere Kundenbewertungen" />
            </label>
            <label>
              Logo-URL:
